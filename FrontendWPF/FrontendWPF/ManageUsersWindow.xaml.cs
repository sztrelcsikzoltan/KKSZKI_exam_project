    ...
    public partial class ManageUsersWindow : Window
    {
        ...
        private void dataGrid0_CellEditEnding(object sender, DataGridCellEditEndingEventArgs e)
        {
            if (e.EditAction == DataGridEditAction.Commit)
            {
                ...
                else if (new_value != "" && new_value != "-1" && changed_property_name == "Username" && new_value.Length < 5)
                {
                    stopMessage = $"The username must be at least 5 charachters long!";
                }
                ...
                else if (new_value !="" && new_value != "-1" && changed_property_name == "Location" && Shared.locationsList.Any(p => p == new_value) == false) // if wrong Location name is entered
                {
                    stopMessage = $"The location '{new_value}' does not exist, please enter the correct location!";
                }
                else if (changed_property_name == "Permission" && (new_value == "" || (new_value != "-1" && Shared.permissionList.Any(p => p == new_value) == false))) // if wrong Permission value is entered
                {
                    stopMessage = $"The Permission value '{new_value}' does not exist, please enter the correct value (between 0-9)!";
                }
                else if (changed_property_name == "Active" && (new_value == "" || (new_value != "-1" && (new_value == "0" || new_value == "1") == false))) // if wrong Active value is entered
                {
                    stopMessage = $"The Active value '{new_value}' does not exist, please enter the correct value (1 or 0)!";
                }
                ...
                */
                ...
                foreach (var user in dbUsersList)
                {
                    ...
                    if ( (user_filter.Id == -1 || user_filter.Id == null || user.Id == user_filter.Id) && (user_filter.Username == "-1" || user_filter.Username == "" || user.Username == user_filter.Username) && (user_filter.Password == "-1" || user_filter.Password == "" || user.Password == user_filter.Password) && (user_filter.Location == "-1" || user_filter.Location == "" || user.Location == user_filter.Location) && (user_filter.Permission == -1 || user_filter.Permission == null || user.Permission == user_filter.Permission) && (user_filter.Active == -1 || user_filter.Active == null || user.Active == user_filter.Active))
                    {
